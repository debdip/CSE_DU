...
def test(input=""):
    ...
    url2 = "token="
    url1 = "token="+h.digest().encode("hex")+m

    h = md5("user=admin&command1=ListFiles&command2=NoOp").hexdigest()
    h = md5(state=h.decode("hex"),count=512)
    extension = "&command3=DeleteAllFiles"
    h.update(extension)
    print h.digest().encode("hex")
    
    padding = chr(128)+chr(0)*(56-len(m)-1)+struct.pack('<q',len(m)*8)
    mess = m+padding+extension
    print md5(mess).hexdigest()
    url2=url2+h.digest().encode("hex")+"&"
    url2=url2+m+padding.encode("hex")+extension
    print "Given url:"+url1;
    print "modified url2:"+url2
    #url12=url12+h.digest().encode("hex")+"&"
    #url12=url12+m+padding.encode("hex")+extension
    #print url11
    #print url12
